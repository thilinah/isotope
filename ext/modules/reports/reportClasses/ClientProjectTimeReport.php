...
class ClientProjectTimeReport extends PDFReportBuilder implements PDFReportBuilderInterface{
    public function getData($report,$request){
        ...
        $timeEntryList = $employeeTimeEntry->Find("employee = ? and date(date_start) >= ? and  date(date_end) <= ? and project in (".implode(",",$projectIds).") order by date_start",
        ...
        foreach($timeEntryList as $timeEntry){
            ...
            $entry->duration = CalendarTools::getTimeDiffInHours($timeEntry->date_start, $timeEntry->date_end);
    ...
    private function isHoliday($date, $countryId){
        ...
        $hd = new HoliDay();
        $allHolidays = $hd->Find("dateh = ?  and country IS NULL",array(date('Y-m-d',strtotime($date))));
        ...
        if(!empty($countryId)){
            $countryHolidays = $hd->Find("dateh = ? and country = ?",array(date('Y-m-d',strtotime($date)), $countryId));
